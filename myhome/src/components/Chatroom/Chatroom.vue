<template>
  <div class="chatroom">
    ...
    <div class="chat-topbar">
      ...
      <div class="clear">
  			<input ref="chatinput" class="chat-input" id="chat-input" v-on:click="getTxt1CursorPosition"  v-on:keyup="getTxt1CursorPosition" type="text" placeholder="enter your message" v-model="chat">
...
export default {
  ...
  watch: {
    '$route':function(to, from) {
      socket.disconnect()//切换路由时可以断开之前的socket
      this.chat = ''
      this.chats = []
      socket = io(config.server,{'force new connection': true})
      this.init()
    }
  },
  created(){
  ...
  methods: {
    init() {
      let that = this
      socket.emit('join',this.$route.params.roomId)
    ...
    refresh(done){
  	...
  	loaddata(num){
  		...
  		socket.emit('load message',num, function(msg){
  			let datalist = msg
